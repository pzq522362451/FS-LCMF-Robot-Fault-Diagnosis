...
SMALL_DATA_DIR = os.path.join(ROOT_DIR, "Data1")
LARGE_DATA_DIR = os.path.join(ROOT_DIR, "Data")
LARGE_REASONING_DIR = os.path.join(LARGE_DATA_DIR, "GPT_Reasoning")

MODEL_PATH = os.path.join(ROOT_DIR, "MainFunc", "Qwen2.5-Omni-7B")
WEIGHT_PATH = os.path.join(
    SMALL_DATA_DIR,
    "qwen_reasoning_lora_checkpoints_S2S3L2",
    "final",
    "lora_weights.pt",
)

SUMMARY_CSV = os.path.join(LARGE_REASONING_DIR, "qwen_reasoning_lora_large_eval_summary.csv")

EVAL_SETS = [
    (
        "large_train",
        os.path.join(LARGE_REASONING_DIR, "large_train_infer_images.jsonl"),
        os.path.join(
            LARGE_REASONING_DIR,
    ...
    (
        "large_test",
        os.path.join(LARGE_REASONING_DIR, "large_test_infer_images.jsonl"),
        os.path.join(
            LARGE_REASONING_DIR,
...
def filter_records(records):
    condition_set = set(EVAL_CONDITIONS or [])
    class_set = set(EVAL_CLASSES or [])
    filtered = []
    for item in records:
        if condition_set and item.get("condition") not in condition_set:
            continue
        if class_set and str(item.get("fault_id", "")).upper() not in class_set:
            continue
...
def resolve_image_path(item):
    ...
    if os.path.isabs(path):
        return path
    return os.path.join(LARGE_DATA_DIR, path)
...
def build_prompt(item, processor):
    ...
    feature_text = ""
    if rms != "" or kurtosis != "":
        feature_text = f"统计特征：RMS={rms}，Kurtosis={kurtosis}\n"

    system_prompt = "你是一个资深的机械故障诊断专家，能够根据RV减速器STFT时频图判断故障类别。"
    user_msg = (
        f"{image_token}\n"
        "请根据该样本进行故障诊断。注意：不能使用真实标签，只能根据图像和可见特征推理。\n"
        f"文件名：{copied_name}\n"
        f"工况：{condition}\n"
        f"{feature_text}"
        f"候选类别包括：\n{candidate_text}\n"
...
def compute_metrics(rows):
    ...
    invalid_by_true = [0 for _ in labels]
    total = 0
    correct = 0
    invalid = 0
    ...
    for row in rows:
        ...
        if pred_label in idx:
            ...
            invalid += 1
            invalid_by_true[idx[true_label]] += 1
    ...
    for i in range(len(labels)):
        ...
        fn = sum(cm[i][c] for c in range(len(labels)) if c != i) + invalid_by_true[i]
...
def run_eval_set(split_name, eval_jsonl, output_jsonl, processor, supports_image, model, device):
    records_all = read_jsonl(eval_jsonl)
    records = filter_records(records_all)
    if len(records) == 0:
...
def main():
    print("=" * 80)
    print("Large-sample Qwen reasoning inference with plain LoRA")
    print(f"Model path: {MODEL_PATH}")
    print(f"LoRA weights: {WEIGHT_PATH}")
    print("Eval sets:")
    for split_name, eval_jsonl, _ in EVAL_SETS:
        print(f"  {split_name}: {eval_jsonl}")
    ...
    summary_rows = []
    for split_name, eval_jsonl, output_jsonl in EVAL_SETS:
        summary_rows.append(
            run_eval_set(split_name, eval_jsonl, output_jsonl, processor, supports_image, model, device)
        )
    save_summary(summary_rows)
    print("=" * 80)
    print(f"Summary saved to: {SUMMARY_CSV}")
    print("Large-sample evaluation finished")
